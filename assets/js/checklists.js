(function($){
    $(document).ready(function(){
        ...
        function renderAccordion(data) {
            ...
            if (!data.length) {
                $accordion.html('<p style="color:#666;"><?php echo addslashes(__('No checklists found for this category.', 'onyx-command')); ?></p>');
                return;
            }
            ...
            data.forEach(function(cl) {
                ...
                // store default state
                if (!state[id]) {
                    var allDone = items.length ? items.every(function(){return false;}) : true;
        ...
        // When page loads, if query param oc_checklist_publish_failed is present show admin notice (WP will redirect with arg)
        if (typeof window.location !== 'undefined' && window.location.search.indexOf('oc_checklist_publish_failed=1') !== -1) {
            var $notice = $('<div class="notice notice-error"><p><?php echo addslashes(__('Publishing blocked: one or more required checklists are incomplete. Complete required checklists before publishing.', 'onyx-command')); ?></p></div>');
